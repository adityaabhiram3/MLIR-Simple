...
/// operations.
struct SimInlinerInterface : public DialectInlinerInterface {
...
SimDialect::SimDialect(mlir::MLIRContext *ctx) : mlir::Dialect("sim", ctx) {
  addOperations<
#define GET_OP_LIST
#include "toy/Ops.cpp.inc"
      >();
  addInterfaces<SimInlinerInterface>();
}
...
void SumOp::build(mlir::OpBuilder &builder, mlir::OperationState &state,
  ...
  //state.addTypes(UnrankedTensorType::get(builder.getF64Type()));
  //state.addTypes(FloatType::get(mlir::StandardTypes::F64, &context));
...
/// Infer the output shape of the SumOp, this is required by the shape inference
/// interface.
void SumOp::inferShapes() {
  //mlir::MLIRContext context;
  //auto arrayTy = getOperand().getType().cast<RankedTensorType>();
  //getResult().setType(FloatType::get(mlir::StandardTypes::F64, &context));
  mlir::MLIRContext context;
  auto arrayTy = getOperand().getType().cast<RankedTensorType>();
  SmallVector<int64_t, 2> dims(1,1);
  getResult().setType(RankedTensorType::get(dims, arrayTy.getElementType()));
}
...
static mlir::LogicalResult verify(MaxOp op) {
...
static mlir::LogicalResult verify(MinOp op) {
...
void ConvOp::inferShapes() {
  ...
  auto filter = getOperand(1).getType().cast<RankedTensorType>();
  int rank = input.getRank();
  if(rank > 3 || filter.getRank() > 2)
    mlir::failure();
  SmallVector<int64_t, 3> dims;
  int i;
  for(i = 0; i < 2; i++) {
    int64_t dims_i = input.getShape()[i] - filter.getShape()[i] + 1;
    dims.push_back(dims_i);
  }
  if(rank == 3) {
    //llvm::errs() << "\n\n rank three shape : "<< input.getShape()[2] << 
    dims.push_back(input.getShape()[2]);
  }
  getResult().setType(RankedTensorType::get(dims, input.getElementType()));
}

static mlir::LogicalResult verify(ConvOp op) {

  auto input = op.getOperand(0).getType().dyn_cast<RankedTensorType>();
  auto filter = op.getOperand(1).getType().dyn_cast<RankedTensorType>();
  auto result = op.getResult().getType().dyn_cast<RankedTensorType>();

  if (!input && !result && !filter)
    return mlir::success();
